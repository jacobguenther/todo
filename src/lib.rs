...
fn get_todofile(read: bool, write: bool, append: bool, truncate: bool, create: bool) -> std::fs::File {
    let mut home = env::var_os("XDG_DATA_HOME").unwrap_or_else(|| env::var_os("HOME").unwrap());
...
fn read_contents() -> String {
    ...
    let _ = BufReader::new(&todofile).read_to_string(&mut contents)
